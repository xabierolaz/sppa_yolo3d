...
\title{Real time Semantic Proxy Reconstruction for UAV Digital Twins:\\
=======
\title{Real-Time Semantic Proxy Reconstruction for UAV Digital Twins:\\
>>>>>>> Stashed changes
...
pose) is cheaper to move by a factor of three to six at stream level---and by
about two orders of magnitude for per-object update messages---but only pays
off if each detection becomes displayable 3D geometry in real time, on the
same machine already running the twin. This paper specifies and validates that missing
...
was measured at \(4\)--\(5\)~m for targets at \(40\)--\(130\)~m range (companion
manuscript, under review; anonymized for double-anonymous review), so
...
twin are validated end to end in the same companion manuscript; the
...
The contribution is \textbf{family-conditioned SPPA}: a real-time
...
algorithm.
Concretely, the paper contributes: (i)~a real-time detection-to-volume
...
Learned primitive decomposition (SuperDec~\cite{fedele2025superdec}, DualPrim~\cite{meng2026dualprim}, superfrusta~\cite{ganeshan2026superfrusta}) & no (image/cloud) & implicit parts & no & GPU inference \\
...
volume. The versioned recipe manifest (ONTOLOGY-0.4) routes detector labels into a
finite set of reviewed archetypes; SPPA can accept an open detector tag at runtime, but the tag is
...
(the versioned ontology manifest); anything outside both becomes the fallback
...
versioned observation record with declared camera/flight geometry and an
...
\begin{figure}[!htb]
\centering
\includegraphics[width=0.95\linewidth,height=0.5\textheight,keepaspectratio]{figures/fig_family_graphs_blender.png}
\caption{The six benchmark family part graphs used by SPPA, panels (a)--(f):
...
Eight primitive slots per family with role-conditioned priors (individual
slots may be degenerate or occluded in a given view); colors denote
semantic roles (bottom legend).}
...
ranges (the full anatomy table is in RP). The slot count is fixed at eight by the shared actor interface, so that
every graph---family or generic---competes at equal capacity; alternative
generic-graph designs at that count were registered in writing and measured
(Section~\ref{sec:generic-sensitivity}).
...
applied to all eight slots at once---\(s_{\mathrm{sec}}\) is a uniform scale
multiplier on the slots flagged \texttt{secondary} in the graph (e.g.,
limbs, wheels) and \(o_{\mathrm{sec}}\) an x-axis translation of those slots
as a fraction of the graph's default extent---, optimizing silhouette agreement over a
...
{\sloppy
A versioned descriptor record carries the resolved family, the role-labeled
part list, and the evidence, uncertainty, and material tags needed to build
the actor once per track; a companion update record separates
...
replay telemetry. An actor is an ordered list of eight primitive slots; the benchmark
instantiates the box/cylinder/ellipsoid subset of the implementation's
primitive types. Family graphs and a single \texttt{generic} graph are
...
+0.5\,(1-\mathrm{IoU}_{\mathrm{side}})+R(\theta)\), with the Tikhonov
regularizer \(R(\theta)=0.01\sum_{i=1}^{3}(\log s_i)^2
+0.005\,(s_{\mathrm{sec}}-1)^2+0.005\,o_{\mathrm{sec}}^2\) keeping the fit
near the recipe prior, evaluated on the actor's projections against the
observed silhouettes.\\[2pt]
...
approximately 94\% normal-approximation power to distinguish a true mean of
...
records are given in RP. All amendments to the sealed
protocol are logged in RP; the two affecting inference are disclosed here:
Amendment~03 substituted the planned external review
...
bootstrap with Holm adjustment). Amendment~01, the deduplication of the
preregistered \texttt{curated\_family\_template} row, is noted in
Table~\ref{tab:paired-deltas}. Development results
...
``real-time'' throughout this paper is a per-object CPU wall-clock claim at
...
(arm \texttt{t5\_drop\_one\_family} in RP) confirms the headline difference is
...
\texttt{sppa\_text\_only}, and \texttt{generic\_mvfit}. Deltas are per-actor
paired means and need not equal differences of the displayed rounded means.}
...
conditions; \(n{=}240\) per condition). Method mean voxel IoU and paired
differences under the five prespecified observation conditions. The prespecified
...
\(+0.130\) (post-hoc bootstrap CI \([0.117,0.143]\); sealed interval
\([0.116,0.144]\) in Table~\ref{tab:paired-deltas}) on the family graph, with a negative
...
few exceptions (the two vehicle families under each other's token, and
vehicle actors fitted with the \texttt{quadruped} token), also below the
generic-graph mean of \(0.367\); the correct-token \texttt{lattice\_tower}
diagonal itself sits below that mean, the sub-voxel resolution effect
discussed in Section~\ref{sec:primary-results}.}
...
generators run at
their fastest official configurations \emph{with} official background removal
...
returns no surface on the tractor flight crop and on the two aerial vehicle
photo crops (car and tractor+trailer; per-case values
in RP). Telemetry
...
when a matched asset exists (shape-stream P95 \(6.8\)~ms vs.\ \(22.4\)~ms
...
baseline (\(0.298\) median versus \(0.330\) for AABB, \(0.423\) for Generic
graph, and \(0.44\)--\(0.45\) for OBB, visual hull, and capsule), and the oriented box
...
error, set by the monocular observation geometry (full analysis in RP). This
long-range monocular regime is not the operating point of the companion
pipeline's \(4\)--\(5\)~m end-to-end error, which is measured at
\(40\)--\(130\)~m range with calibrated geometry; the two figures describe
different sensing regimes and are not directly comparable.
...
Four threats to validity remain binding.
...
\item \emph{Runtime and stream.} ``Real-time'' is per object (millisecond
