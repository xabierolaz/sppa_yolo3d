...
\begin{abstract}
Remote UAV piloting beyond visual line of sight increasingly relies on a
geospatial digital twin: the operator flies from a ground station that
renders an Unreal/Cesium 3D Tiles twin---potentially through an immersive VR
headset---because a live video feed cannot be trusted as the sole visual
channel. Link bandwidth, latency, and visibility degrade with distance,
weather, and darkness, and night or low-visibility operation removes the
camera as a primary sensor altogether. The twin survives those conditions,
but it is stale: the operationally critical delta---a new or relocated
tower, vehicles, people,
animals near the flight path---is exactly what the static layer cannot
contain. Object-level semantic telemetry (detector tags, tracks, footprints,
and aircraft pose) is orders of magnitude cheaper to downlink than video, so
the twin can be kept current if---and only if---each detection can be turned
into displayable 3D geometry instantly, on the same laptop that is already
running the twin. We study instant per-object semantic proxy reconstruction
for that delta: one detection compiles into a role-labeled eight-part
primitive actor (Semantic Primitive Proxy Assembly, SPPA-MVFit) in
\(9.4\)~ms median CPU, is emitted as a \(1.45\)~kB update descriptor, and
updates the twin in place at telemetry bandwidth as the observation
degrades. In preregistered evidence on
...
\(0.847\) \([0.792,0.898]\) on 217 matched cases). Operation is
...
scope; in the companion replay the end-to-end detector-to-twin
georeferencing error was measured at \(4\)--\(5\)~m for targets at
\(40\)--\(130\)~m range, so the reconstructed actors are
awareness-grade volumes, not maneuvering-grade survey---and what the mission
requires is reconstruction fidelity under degraded sensing: does a frozen semantic-family part graph improve lightweight
...
the corrupted observations that a weak image link implies?
...
row of the table fails at least one of the four. SPPA-MVFit is designed for
...
When calibrated top and side silhouettes are available, SPPA-MVFit refines
...
Generic-MVFit by morphology family and source stratum (\(n{=}240\)). The
...
RP). SPPA-MVFit exceeds SPPA text-only (mean
...
reading: \(88.7\%\) of SPPA-MVFit cases still improve in the final sweep and
its \(\theta\) never sits at the frozen bounds, whereas Generic-MVFit pins
...
\paragraph{Representation versus fitting: the graph-by-fitting decomposition.}
...
are Generic-MVFit with its own slot mapping and two frozen visual-hull
...
SPPA-MVFit reaches \(0.561\) mean voxel IoU at \(9.2\)~ms CPU with a
...
\begin{figure}[!htb]
\centering
\includegraphics[width=0.85\linewidth,height=0.5\textheight,keepaspectratio]{figures/fig_hero_fastmode.png}
\caption{The operating-point argument in one view: semantic telemetry in,
displayable geometry out---SPPA-MVFit against six current open 3D generators
...
count, and peak VRAM on the same RTX~5090 workstation. Neural columns are
qualitative context, not a ranking; measured IoU comparisons are the sealed
synthetic wave of Section~\ref{sec:neural-external-wave}.}
...
truth, so 3D occupancy cannot
be scored here; what the stream adds is the evidence-versus-prior
...
the 2--15~Hz stream rate. Localization is outside this paper's scope: all
methods share the same \(\approx\)\(33\)~m median 3D error, set by the
monocular observation geometry (full analysis in
RP).
...
with AUC \(0.847\) (95\% CI \([0.792,0.898]\)), and prior mismatch separates
them perfectly (AUC \(1.000\)), in
the direction opposite to intuition: the
...
(\(\approx\)\(5\)~m observed against a \(25\)~m prior)---a no-observed-overlap result, not a
population claim. A frozen routing policy on these signals
(registered 2026-07-19 before any outcome; RP) turns the signal into a
measured coverage decision:
...
degraded operation exists. The boundary analyses are as informative as the headline:
a wrong family token is worse than no prior at all, making fallback routing
a measured requirement;
adversarial violations of the structural prior destroy a measurable share of
the advantage; and on external real meshes the margin does not
...
select---rather than volumetric leadership. The real detector stream
exercises the same regime end to end within its latency budget, with
detector-token confidence and prior fit as measured validation signals for
accepting, refining, or rejecting a family token; two further exploratory
...
Within these boundaries the supported claim is narrow: family conditioning
improves occupancy beyond the prespecified margin on the held-out synthetic
test, and SPPA descriptors are consumable by an optional Unreal backend;
real-UAV accuracy, universal open-set geometry, operator benefit, and neural
image-to-3D ranking remain outside the claim.
Four directions are left to future work. First, the frozen coordinate-descent
...
detector-derived silhouettes. Third, the
...
SPPA-MVFit targets the moment a UAV digital twin must be updated under a
...
volumes at once. This paper has shown that a reviewed semantic part graph is
a measurable occupancy prior for that reconstruction: the
preregistered equal-budget comparison passes its prespecified margin and
holds under every preregistered corruption, and the decomposition attributes most of the
advantage to the representation rather than the fitter. The
...
token-validation signals, and on two real flight
...
cost---and its boundary is measured with
equal care: a wrong family token is worse
than none, adversarial violations destroy part of the advantage, and the
margin does not replicate on external meshes at
this sample size, so the contract routes, refines, or falls back rather
than hallucinating. The durable contribution is the twin-delta update
...
value. Operation is camera-validated today and position is assumed given;
the next steps are
...
{\sloppy
The SPPA-MVFit reproducibility package (preregistration,
